...
public class Pair<FirstType, SecondType>{
    ...
    public void ifPresent(BiConsumer<? super FirstType, ? super SecondType> consumer){
        if ( first.isPresent() && second.isPresent() ) consumer.accept(first.get(), second.get());
    }

    @Override
    public boolean equals(Object o){
        if ( this == o ) return true;Pair<FirstType, SecondType> pair = (Pair<FirstType, SecondType>) o;
        if ( pair == null ) return false;return first.equals(pair.first) && second.equals(pair.second);
    }
